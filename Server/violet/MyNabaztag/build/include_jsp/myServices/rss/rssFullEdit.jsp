...
<div class="main-cadre-contener">
	...
	<div class="main-cadre-content">
		<%-- ************************************************************************************************************* --%>	
		<bean:define id="subscriptionId" name="mySrvRssFullForm" property="subscriptionId" type="Long"/>
		<bean:define id="dispatch" name="mySrvRssFullForm" property="dispatch" type="String"/>
		<bean:define id="name" name="mySrvRssFullForm" property="name" type="String"/>
		<bean:define id="rssNbNews" name="mySrvRssFullForm" property="srvNbNews" type="Integer"/>
		
		<html:form action="/action/srvRssFullConfig" styleId="srvRssFreeConfig" styleClass="srvConfigForm" >
			...
			<ul class="radio-chooser">
				<li>
					<html:radio styleId="confHeure" name="mySrvRssFullForm" property="srvModeListener" value="1" /><span><%=DicoTools.dico(dico_lang, "srv_rss/week_and_week_end")%></span>
					<label class="big"><%=DicoTools.dico(dico_lang, "srv_rss/choose_time_week")%></label>
					<input id="time1" class="hourPicker" type="hidden" name="scheduleW" value="<bean:write name="mySrvRssFullForm" property="scheduleW"/>">
					
					<hr class="spacer">
					
					<label class="big"><%=DicoTools.dico(dico_lang, "srv_rss/choose_time_week_end")%></label>									
					<input id="time2" class="hourPicker" type="hidden" name="scheduleWE" value="<bean:write name="mySrvRssFullForm" property="scheduleWE"/>"><br/>				
				</li>
				<li>
					<html:radio  styleId="confDelai"  name="mySrvRssFullForm" property="srvModeListener" value="2" /><%=DicoTools.dico(dico_lang, "srv_rss/everytime_new")%>
					<html:select name="mySrvRssFullForm" property="srvFrequencyListening">
...
<script type="text/javascript">
	...
	$("#srvRssFreeConfig").submit(function(){
		return nablifeValidateRssFreeConfig(true);		
	});
