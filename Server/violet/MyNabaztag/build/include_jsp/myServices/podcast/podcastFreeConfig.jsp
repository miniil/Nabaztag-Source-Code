...
<div class="main-cadre-contener">
	...
	<div class="main-cadre-content">
		<%-- ************************************************************************************************************* --%>	
		<bean:define id="applicationId" name="mySrvPodcastFreeForm" property="applicationId" />
		<bean:define id="subscriptionId" name="mySrvPodcastFreeForm" property="subscriptionId" />
		<bean:define id="dispatch" name="mySrvPodcastFreeForm" property="dispatch" type="String"/>
		<bean:define id="subscribed" name="mySrvPodcastFreeForm" property="subscribed" />
		<bean:define id="podcastNbNews" name="mySrvPodcastFreeForm" property="srvNbNews" type="Integer"/>
		
		<html:form action="/action/srvPodcastFreeConfig" styleId="srvPodcastFreeConfig" styleClass="srvConfigForm" >
			...
			<ul class="radio-chooser">
				<li>
					<html:radio styleId="confHeure" name="mySrvPodcastFreeForm" property="srvModeListener" value="1" /><span><%=DicoTools.dico(dico_lang, "srv_rss/week_and_week_end")%></span>
					<label class="big"><%=DicoTools.dico(dico_lang, "srv_rss/choose_time_week")%></label>
					<input id="time1" class="hourPicker" type="hidden" name="scheduleW" value="<bean:write name="mySrvPodcastFreeForm" property="scheduleW"/>">
					
					<hr class="spacer">
					
					<label class="big"><%=DicoTools.dico(dico_lang, "srv_rss/choose_time_week_end")%></label>									
					<input id="time2" class="hourPicker" type="hidden" name="scheduleWE" value="<bean:write name="mySrvPodcastFreeForm" property="scheduleWE"/>"><br/>				
				</li>
				<li>
					<html:radio  styleId="confDelai"  name="mySrvPodcastFreeForm" property="srvModeListener" value="2" /><%=DicoTools.dico(dico_lang, "srv_rss/everytime_new")%>
					<html:select name="mySrvPodcastFreeForm" property="srvFrequencyListening">
						<html:optionsCollection name="mySrvPodcastFreeForm" property="periodList" label="label" value="id"/>
					</html:select>				
				</li>
			</ul>
			<hr class="spacer">
...
<script type="text/javascript">
	...
	$("#srvPodcastFreeConfig").submit(function(){
		return nablifeValidatePodcastFreeConfig(<logic:equal name="subscribed" value="true" >true</logic:equal>);		
	});
	
	<logic:equal name="mySrvPodcastFreeForm" property="isV1" value="true">
