...
public final class Vasm {
	...
	public enum VASM_FILE {
		V1(HARDWARE.V1), // label = Nabaztag
		DALDAL(HARDWARE.DALDAL); // label = Daldal
		...
		private VASM_FILE(HARDWARE inHardware) {
			...
			final String theLabel = inHardware.getLabel();
			this.bluePointFileName = theLabel + net.violet.common.StringShop.UNDERSCORE + Vasm.POINTBLEU;
			this.demoFileName = theLabel + net.violet.common.StringShop.UNDERSCORE + Vasm.DEMO;
			this.waitFileName = theLabel + net.violet.common.StringShop.UNDERSCORE + Vasm.WAIT;
			this.sleepFileName = theLabel + net.violet.common.StringShop.UNDERSCORE + Vasm.ASLEEP;
			this.msgFileName = theLabel + net.violet.common.StringShop.UNDERSCORE + Vasm.MSG;
		}
	...
	private static byte[] getfilebytes(String filename) throws IOException {
		...
		if (net.violet.common.StringShop.EMPTY_STRING.equals(filename) || filename.startsWith(Files.CATEGORIES.BROADCAST.getCategory()) || (filename.charAt(1) == ':')) {
			theResult = FilesManagerFactory.FILE_MANAGER.getFilesContent(filename);
		} else {
	...
	private int cutline(String src, int inI0, String[] words, int n) {
		...
		while (k < n) {
			words[k++] = StringShop.EMPTY_STRING;
		}
		go = 1;
		while (go > 0) {
			if (i >= l) {
	...
	static String[] cutvirg(String src, AssoVasm equ) {
		...
		if (theArgsStrTrimmed.equals(StringShop.EMPTY_STRING)) {
			theArgs = new String[0];
		} else {
			theArgs = theArgsStrTrimmed.split(StringShop.COMMA);
		...
		try {
			...
			while (i < src.length()) {
				...
				if ((words[0].length() == 0) || (words[0].charAt(0) != ';')) {
					...
					if ((words[0].length() != 0) && (words[0].charAt(0) == '/') && (words[1].compareToIgnoreCase("FILE") == 0)) {
